class AppointmentsController < ApplicationController
  ...
  def check
    ...
    redirect_to hospitals_path flash.now[:error] = "No Appointment found"
  ...
  def book
    if current_account
      ...
      p "hai"
      redirect_to hospitals_path, alert: "Login to Book Appointment"
    end
  end
  ...
  def decline
    ...
    if @appointment.update(status)
      redirect_to doctor_appointments_path(current_account.accountable_id) , notice: "successfully declined"
    end
  end
  ...
  def update
    ...
    status['status'] = 1
    if @appointment.update(status)
      redirect_to doctor_appointments_path(current_account.accountable_id) , notice: "Booking Confirmed"
    end
  end
  ...
  def destroy
    ...
    if @appointment.destroy
      ...
      elsif redirect_to doctor_appointments_path(current_account.accountable_id), notice: 'Deleted Successfully'
      end
